...
class CoreSetting(models.Model):
    ...
    whats_new = RichTextField(blank=True,null=True)

    def can_invite(self):
        try:
            if CoreSetting.objects.get(pk=1).freeze_team_invites:
                return False
            else:
                return True
        except:
    ...
    def can_create_team(self):
        try:
            if CoreSetting.objects.get(pk=1).disable_team_creation:
                return False
            else:
                return True
        except:
...
class StaticInfo(models.Model):
    ...
    featured_competence = models.ForeignKey(Competition, related_name='featured_competence',
