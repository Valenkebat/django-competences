...
from .models import StaticInfo, Partner, CoreSetting, StaticPage
...
def index(request):
    ...
    tournament_list = Competition.objects.filter(active=True).order_by('-id')[:8]
...
def whats_new(request):
    olly = CoreSetting.objects.get(pk=1)
