...
def add_teams(request, pk):
    ...
    if user.user_type not in allowed:
        ...
        if request.method == 'POST':
            tournament = SingleEliminationCompetition.objects.get(pk=pk)
...
def tournaments(request):
    ...
    if user.user_type not in allowed:
        ...
        tournament_list = SingleEliminationCompetition.objects.all().order_by('-id')
...
def tournament_detail(request, pk):
    ...
    if user.user_type not in allowed:
        ...
        tournament = SingleEliminationCompetition.objects.get(pk=pk)
        rounds = SingleEliminationRound.objects.filter(tournament=tournament).order_by('id')
...
def round_detail(request, pk):
    ...
    if user.user_type not in allowed:
        ...
        tround = SingleEliminationRound.objects.get(pk=pk)
...
def edit_round(request, pk):
    ...
    if user.user_type not in allowed:
        ...
        if request.method == 'POST':
            roundobj = SingleEliminationRound.objects.get(id=pk)
...
def edit_tournament(request, pk):
    ...
    if user.user_type not in allowed:
        ...
        if request.method == 'POST':
            tournamentobj = SingleEliminationRound.objects.get(pk=pk)
            ...
            tournamentobj = SingleEliminationCompetition.objects.get(pk=pk)
            time = datetime.datetime.utcnow()
...
def ruleset_list(request):
    ...
    if user.user_type not in allowed:
        ...
        if request.method == 'GET':
            rulesets = Competition.objects.all().order_by('id')
...
def ruleset_detail(request, pk):
    ...
    if user.user_type not in allowed:
        ...
        if request.method == 'POST':
            ...
            ruleset = Competition.objects.get(id=pk)
...
def mikes_super_function(currentround):
    currentround = SingleEliminationCompetition.objects.get(pk=currentround)
...
def advance(request, pk):
    ...
    if user.user_type not in allowed:
        ...
        tournament = SingleEliminationCompetition.objects.get(pk=pk)
        currentround = SingleEliminationCompetition.objects.get(tournament=tournament, roundnum=tournament.current_round)
        nextround = SingleEliminationCompetition(tournament=tournament, roundnum=tournament.current_round+1)
