...
from pages.models import FrontPageSlide, CoreSetting, StaticPage
...
def staffindex(request):
    ...
    if user.user_type not in allowed:
        ...
        return render(request, 'staff/staffindex.html', {'ticket': ticket, 'news': news, 'teams': teams,
                                                         'tournaments': competitions, 'numusers': numusers,
...
def create_settings(request):
    ...
    if user.user_type not in allowed:
        ...
        if len(CoreSetting.objects.all()) > 1:
...
def edit_settings(request):
    ...
    if user.user_type not in allowed:
        ...
        settings = CoreSetting.objects.get(pk=1)
        if request.method == 'POST':
            form = CreateOllySetting(request.POST)
            if form.is_valid():
